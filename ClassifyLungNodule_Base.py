...
X, Y = create_placeholders(448, 448, 1, 3)
print ("X = " + str(X))
print ("Y = " + str(Y))
...
with tf.Session() as sess:
    ...
    init = tf.global_variables_initializer()
    sess.run(init)
    a = sess.run(Z3, {X: np.random.randn(2,448,448,1), Y: np.random.randn(2,3)})
    print("Z3 = " + str(a))
...
with tf.Session() as sess:
    ...
    init = tf.global_variables_initializer()
    sess.run(init)
    a = sess.run(cost, {X: np.random.randn(4,448,448,1), Y: np.random.randn(4,3)})
    print("cost = " + str(a))
