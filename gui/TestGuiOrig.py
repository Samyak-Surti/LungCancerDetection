...
class App:
    ...
    def classify(self):
	    ...
	    pneumo_probabilities = self.classify_pneumonet(self.imagePath)
	    self.label_print('Computed probabilities:')
	    for pneumo_category in pneumo_probabilites:
		self.label_print('  ' + pneumo_category)
	...
	self.label_print('Reading tensor from image file...')
	img = image[0]
	img = read_tensor_from_image_file_pneumo(image,
